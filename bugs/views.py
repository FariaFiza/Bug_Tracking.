...
def home(request):
    stats = {
        ...
        'resolved_bugs': BugReport.objects.filter(status='fixed').count(),
        'total_devs': CustomUser.objects.filter(role='developer').count(),
    }
    recent_bugs = BugReport.objects.select_related('submitted_by', 'assigned_developer')[:5]
    feedbacks = Feedback.objects.filter(is_public=True).select_related('submitted_by')[:4]
...
def bug_list(request):
    ...
    if status:
        # HTML এর 'open' এবং 'resolved' কে মডেল চয়েসের সাথে ম্যাপ করা হলো
        if status == 'open':
            bugs = bugs.filter(status='pending')
        elif status == 'resolved':
            bugs = bugs.filter(status='fixed')
        else:
...
def bug_create(request):
    if request.method == 'POST':
        # স্ক্রিনশট ও ফাইল আপলোডের জন্য request.FILES যুক্ত করা হয়েছে
...
def bug_edit(request, pk):
    ...
    if bug.submitted_by != request.user and not request.user.is_admin_user():
        ...
        return redirect('bug_detail', pk=pk)
    if request.method == 'POST':
        # এখানেও request.FILES যুক্ত করা হয়েছে
